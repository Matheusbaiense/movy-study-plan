        ...
        <thead>
          <tr style={{ borderBottom: '1px solid rgba(0,0,0,0.1)' }}>
            {['Date', 'Clock In', 'Clock Out', 'Hours', 'Day Type', 'Amount', 'Status', 'Actions'].map((h) => (
              <th key={h} style={{ padding: '8px 12px', textAlign: 'left', fontWeight: 600, color: '#888', fontSize: 11, letterSpacing: '0.06em', textTransform: 'uppercase' }}>
                {h}
              </th>
            ))}
          </tr>
          ...
          {entries.map((e) => {
            ...
            return (
              <tr key={e.id} style={{ borderBottom: '1px solid rgba(0,0,0,0.06)' }}>
                ...
                <td style={{ padding: '10px 12px', color: '#888', fontVariantNumeric: 'tabular-nums' }}>
                  {e.clock_out
