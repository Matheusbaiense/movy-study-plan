...
export default async function HomePage({ params }: Props) {
  ...
  return (
    <div className="movy-stagger" style={{ display: 'grid', gap: 34, paddingTop: 4 }}>
      {/* Masthead */}
      <header>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 16, flexWrap: 'wrap' }}>
          <span className="movy-kicker">Movy Internal Hub</span>
          <span style={{ fontFamily: font.mono, fontSize: 11, color: ink(0.42), letterSpacing: '0.04em' }}>{today} · PERTH</span>
        </div>
        <hr className="movy-rule" style={{ margin: '14px 0 24px' }} />
        <h1 style={{ margin: 0, fontFamily: font.display, fontSize: 'clamp(44px, 7vw, 84px)', fontWeight: 800, letterSpacing: '-0.045em', lineHeight: 0.9, color: color.purpleDeep }}>
          {greeting(isEn)}<span style={{ color: color.gold }}>.</span>
        </h1>
        <p style={{ margin: '16px 0 0', maxWidth: 540, color: color.inkSoft, fontSize: 18, fontWeight: 500, lineHeight: 1.55 }}>
          {isEn ? <>Welcome back, <strong style={{ color: color.purpleDeep, fontWeight: 700 }}>{firstName}</strong>.</>
            : <>Bem-vindo de volta, <strong style={{ color: color.purpleDeep, fontWeight: 700 }}>{firstName}</strong>.</>}{' '}
      ...
      <section style={{ display: 'grid', gap: 16 }} className="grid-cols-1 lg:grid-cols-2">
        ...
        <div style={{ position: 'relative', overflow: 'hidden', borderRadius: 16, background: color.purpleDeeper, color: '#fff', border: `1px solid ${color.purpleDeep}` }}>
          ...
          <div style={{ position: 'relative', padding: '28px 30px 30px', minHeight: 232, display: 'flex', flexDirection: 'column' }}>
            ...
            <h2 style={{ margin: '14px 0 8px', fontFamily: font.display, fontSize: 'clamp(26px, 3.2vw, 34px)', fontWeight: 800, letterSpacing: '-0.035em', lineHeight: 0.96, maxWidth: 320 }}>
        ...
        {/* Capacidade Financeira — light, structural */}
        <Link href={`/${locale}/financial`} prefetch={false} className="movy-card movy-card--hover" style={{ position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', padding: '28px 30px 30px', minHeight: 232, textDecoration: 'none' }}>
          <svg viewBox="0 0 120 120" width={210} height={210} aria-hidden style={{ position: 'absolute', right: -22, bottom: -34, color: color.purple, opacity: 0.07 }}>
            <use href="#movySymMono" />
          </svg>
          ...
          <h2 style={{ margin: '14px 0 8px', fontFamily: font.display, fontSize: 'clamp(26px, 3.2vw, 34px)', fontWeight: 800, letterSpacing: '-0.035em', lineHeight: 0.96, color: color.purpleDeep, maxWidth: 300 }}>
            {isEn ? 'Financial capacity' : 'Capacidade financeira'}
          </h2>
          <p style={{ margin: '0 0 22px', maxWidth: 320, color: ink(0.6), fontSize: 14, lineHeight: 1.55 }}>
          ...
          <span style={{ marginTop: 'auto', display: 'inline-flex', alignItems: 'center', gap: 8, color: color.purpleDeep, fontFamily: font.ui, fontWeight: 700, fontSize: 14 }}>
      ...
      {/* CRM — coming soon */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 18, padding: '18px 22px', borderRadius: 14, background: color.lilac, border: `1px solid ${color.line}`, flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: 200 }}>
          <span className="movy-kicker" style={{ color: ink(0.4) }}>{isEn ? 'Coming soon · CRM' : 'Em breve · CRM'}</span>
          <div style={{ marginTop: 8, fontFamily: font.display, fontSize: 18, fontWeight: 800, color: color.purpleDeep, letterSpacing: '-0.015em' }}>
            {isEn ? 'Student & lead management' : 'Gestão de leads e estudantes'}
          </div>
          <p style={{ margin: '4px 0 0', color: ink(0.55), fontSize: 13.5, lineHeight: 1.5, maxWidth: 560 }}>
        ...
        <span style={{ fontFamily: font.mono, fontSize: 11, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: color.orange, border: `1px solid ${color.line}`, background: '#fff', padding: '6px 12px', borderRadius: 999 }}>
...
const ghostBtn: React.CSSProperties = {
  ...
  textDecoration: 'none', fontSize: 14, fontFamily: 'Outfit, system-ui, sans-serif',
