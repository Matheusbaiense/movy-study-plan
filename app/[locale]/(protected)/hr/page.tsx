...
export default async function HrPage({ params }: Props) {
  ...
  const { weekStart, weekLabel } = getWeekBounds()
  ...
  const employeeNameMap: Record<string, string> = Object.fromEntries(
    allEmployees.map(e => [e.id, e.full_name || e.email || e.id.slice(0, 8)])
  )

  return (
    <div style={{ padding: '24px 32px', maxWidth: 1400, margin: '0 auto' }}>
      <PageHeader
        eyebrow={`${locale === 'pt' ? 'Operações' : 'Operations'} › HR`}
        title={locale === 'pt' ? 'RH & Controle de Horas' : 'HR & Time Management'}
      />
