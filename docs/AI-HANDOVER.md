...
> **Pesquisa AllyHub** (2026-06-15/16, 12 sessões): extraída para `.wolf/allyhub-research/AI-HANDOVER-ALLYHUB.md`. Blueprint completo em `.wolf/allyhub-research/ALLY-BLUEPRINT.md` (~4400+ linhas).
...
> Sem migration (usa schema do 6A). **Próximo = SPLIT 5 (aceite/assinatura de proposta).**

- **`app/[locale]/(protected)/portfolio/page.tsx`** (RSC): lista instituições com contagem de cursos → `PortfolioPage`.
- **`app/[locale]/(protected)/portfolio/[institutionId]/page.tsx`** (RSC): detalhe da instituição com cursos + vigências + regras → `InstitutionDetail`.
- **`app/[locale]/(protected)/portfolio/actions.ts`** (9 server actions): `createInstitutionAction`, `updateInstitutionAction`, `archiveInstitutionAction`, `createCourseAction`, `updateCourseAction`, `toggleCourseActiveAction`, `archiveCourseAction`, `createPriceVersionAction`, `createPricingRuleAction`, `togglePricingRuleAction`, `deletePricingRuleAction`. Todas com `requireAdmin`/`requireEditor` guard + logAudit.
- **`components/portfolio/PortfolioPage.tsx`**: grid de cards por instituição, busca por nome/cidade/país, flag de país (emoji alpha-2), badge de parceria (Ativo/Negociando/Inativo), badge de vencimento de preços (≤30d), modal inline de criação/edição.
- **`components/portfolio/InstitutionDetail.tsx`**: 3 abas:
  - **Cursos** — lista com badge de tipo (ELICOS/VET/HE), toggle ativo/inativo, archive, modal de criação/edição.
  - **Vigências** — agrupado por curso ativo; tabela de `course_price_versions` (início/fim/matrícula/taxa/material/depósito) + badge de vencida/vencendo; modal de nova vigência (entradas em AUD, convertidas para cents).
  - **Regras** — lista de `pricing_rules` com toggle ativo/arquivar; modal de nova regra (escopo, efeito, valor, validade).
- **`components/layout/AppShell.tsx`**: nav entry "Portfólio" (Building2 icon) adicionado entre Propostas e Financeiro.

**Fora de escopo (backlog):** campus management UI, alertas de impacto ("X propostas usam este preço"), duplicar escola, promotions dedicadas.

### 2026-06-16 - SPLIT 4 · fatia C: migration 012 + BRL/ExplainPanel + VersionHistory — **SPLIT 4 COMPLETO**

> **Estado:** SPLIT 4 **100% entregue**. type-check ✅ · `node --test` 64/64 ✅ · build ✅.
> Migration 012 aplicada em `xpthmguzcbmndyyexfbt`. **Próximo = SPLIT 6B (portfólio UI).**

**O que foi entregue nesta fatia (AllyHub gap-filling incluso):**

- **Migration 012 aplicada** (`supabase/migrations/012_proposal_templates_versions.sql`):
  - `proposal_templates` — org_id, name, applicant_type, description, data jsonb, is_active, metadata; RLS (read=active users, insert/update=editors+, delete=admins); unicidade `(org_id, name)`.
  - `proposal_versions` — **IMUTÁVEL** (sem UPDATE policy): org_id, study_plan_id, version_number (trigger BEFORE INSERT seta atômico por plano), label, reason (`manual`/`status_change`/`restore`), status, data+computed jsonb, created_by, created_at apenas. Garante versionamento histórico sem corrida de numeração.
  - `types/supabase.ts` regenerado do banco vivo (58 007 chars).

- **`EditorStickyBar.tsx`** (reescrito, 170 linhas) — 2 features novas:
  - **BRL equivalente:** `formatBRL(audCents, fxRate)` → `≈ R$ X.XXX` abaixo do grand total. Rate vem do pai via prop `fxRate?: number | null`.
  - **ExplainPanel:** clique no chip Total abre painel `sp-explain-panel` (CSS slideUp 140ms) com grid 3 colunas — por curso (nome | detalhes taxa) + extras + grand total. Fecha em segundo clique. Props novas: `courses?: CourseSlice[]`, `extraCosts?: ExtraCost[]`.

- **`StudyPlanEditor.tsx`** — wiring central:
  - `useEffect` busca `/api/fx` no mount → `fxRate` em state; passado ao `EditorStickyBar`.
  - Botão "Versões" no header toggle `showVersions`; painel `<VersionHistory>` condicional.
  - `EditorStickyBar` recebe `fxRate`, `courses` (mapeados para `CourseSlice`) e `extraCosts`.
  - CSS `sp-explain-panel` + `@keyframes sp-explain-in` no `editorStyles` do componente.

- **`VersionHistory.tsx`** (novo, ~180 linhas) — UI de versões:
  - Lista últimas 20 versões: `v{number}`, label opcional, badge de reason, tempo relativo.
  - Formulário inline "Salvar versão" com label opcional; Enter para submeter.
  - Restore com duplo-clique (primeiro = "Confirmar?", segundo = executa) para evitar acidentais.
  - Callback `onRestored` → `window.location.reload()` no editor.

- **Server actions** (5 novas em `actions.ts` antes de `deleteStudyPlan`):
  - `saveVersionAction(planId, label?)` — lê `data` + `computed` do plano, insere com `version_number=0` (trigger reescreve), emite evento `version_saved`.
  - `listVersionsAction(planId)` → `VersionSummary[]` (últimas 20, desc).
  - `restoreVersionAction(planId, versionId)` — snapshota estado atual como `reason='restore'` antes de sobrescrever; restaura `version.data` via `withComputed()`; `revalidatePath`.
  - `listTemplatesAction()` → `TemplateSummary[]` (todos ativos do org).
  - `saveAsTemplateAction(planId, name, description?)` — strip PII (mantém cursos/extraCosts/tipos), insere em `proposal_templates`.
  - Todas as conversões `Json → StudyPlanData` usam `as unknown as StudyPlanData` (padrão do projeto).

**Vantagem competitiva vs AllyHub:** AllyHub não tem templates, não tem histórico de versões e tem bug de OSHC vencido bloqueando todas as cotações AU. Movy entrega os três.

**Backlog (fora do SPLIT 4):** botão "Salvar como template" na UI do editor; campo data-validade no template; landing page da proposta (mídia/like/WhatsApp); comissão; payment plan por opção.

**Próximo split:** SPLIT 6B — telas de CRUD de portfólio (instituições/campus/cursos/vigências).
